...
|-----------|---------------------------------------------------------------|----------|------|
| 아이템 1  | 생성자 대신 정적 팩터리 메서드를 고려하라                       | 김대화   | 250713|
| 아이템 2  | 생성자에 매개변수가 많다면 빌더를 고려하라                     | 이민수   |250713|
| 아이템 3  | private 생성자나 열거 타입으로 싱글턴임을 보증하라             | 신승민   |250713|
| 아이템 4  | 인스턴스화를 막으려거든 private 생성자를 사용하라              | 이우용   |250713|
| 아이템 5  | 자원을 직접 명시하지 말고 의존 객체 주입을 사용하라            | 김태현   |250713|
| 아이템 6  | 불필요한 객체 생성을 피하라                                     | 오승엽   |250713|
| 아이템 7  | 다 쓴 객체 참조를 해제하라                                     | 이상수   |250713|
| 아이템 8  | finalizer와 cleaner 사용을 피하라                              | 조율기   |250713|
| 아이템 9  | Try-finally보다는 try-with-resources를 사용하라               | 김대화   |250720|
...
| 아이템 13 | Clone 재정의는 주의해서 진행하라                               | 김태현   |250720|
| 아이템 14 | Comparable을 구현할 지 고려하라                                | 오승엽   |250720|
...
| 아이템 22 | 인터페이스는 타입을 정의하는 용도로만 사용하라                | 오승엽   |250727|
| 아이템 23 | 태그 달린 클래스보다는 클래스 계층구조를 활용하라             | 이상수   |250727|
| 아이템 24 | 멤버 클래스는 되도록 static으로 만들라                         | 조율기   |250727|
| 아이템 25 | 톱레벨 클래스는 한 파일에 하나만 담으라                        | 김대화   |250803|
...
| 아이템 30 | 이왕이면 제네릭 메서드로 만들라                               | 오승엽   |250803|
| 아이템 31 | 한정적 와일드카드를 사용해 API 유연성을 높이라               | 이상수   |250803|
| 아이템 32 | 제네릭과 가변인수를 함께 쓸 때는 신중하라                     | 조율기   |250803|
| 아이템 33 | 타입 안전 이종 컨테이너를 고려하라                            | 김대화   |250810|
...
| 아이템 39 | 명명 패턴보다 애너테이션을 사용하라                           | 이상수   |250810|
| 아이템 40 | @Override 애너테이션을 일관되게 사용하라                      | 조율기   |250810|
...
| 아이템 47 | 반환 타입으로는 스트림보다 컬렉션이 낫다                       | 이상수   |250817|
| 아이템 48 | 스트림 병렬화는 주의해서 적용하라                             | 조율기   |250817|
...
| 아이템 55 | 옵셔널 반환은 신중히 하라                                     | 이상수   |250824|
| 아이템 56 | 공개된 API 요소에는 항상 문서화 주석을 작성하라              | 조율기   |250824|
...
| 아이템 63 | 문자열 연결은 느리지 주의하라                                 | 이상수   |250831|
| 아이템 64 | 객체는 인터페이스를 사용해 참조하라                          | 조율기   |250831|
...
| 아이템 71 | 필요 없는 검사 예외 사용은 피하라                             | 이상수   |250907|
| 아이템 72 | 표준 예외를 사용하라                                          | 조율기   |250907|
...
| 아이템 79 | 과도한 동기화는 피하라                                        | 이상수   |250914|
| 아이템 80 | 스레드보다는 실행자, 테스크, 스트림을 애용하라               | 조율기   |250914|
...
| 아이템 87 | 커스텀 직렬화 형태를 고려해보라                               | 이상수   |250921|
| 아이템 88 | readObject 메서드는 방어적으로 작성하라                      | 조율기   |250921|
